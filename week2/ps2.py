...
class RectangularRoom(object):
    ...
    def __init__(self, width, height):
        ...
        self.width = width
        self.height = height
        self.tiles = {(x, y): RectangularRoom.DIRTY for x in range(width) for y in range(height)}
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        if self.tiles[(int(pos.getX()), int(pos.getY()))] == RectangularRoom.DIRTY:
            self.tiles[(int(pos.getX()), int(pos.getY()))] = RectangularRoom.CLEAN
    ...
    def isTileCleaned(self, m, n):
        ...
        """
        if self.tiles[(m, n)] == RectangularRoom.CLEAN:
            return True
        return False
    ...
    def isPositionInRoom(self, pos):
        ...
        posX = pos.getX()
        posY = pos.getY()
        return (math.floor(posX), math.floor(posY)) in self.tiles  
...
class Robot(object):
    ...
    def __init__(self, room, speed):
        ...
        randomX = random.random() * room.width
        randomY = random.random() * room.height
        self.position = Position(randomX, randomY)
    ...
    for i in xrange(num_trials):
        ...
        while True:
            ...
            if room.getNumCleanedTiles() / float(room.getNumTiles()) >= min_coverage:
                stepsNeededList.append(steps)
                break
            else:
                steps += 1
    return sum(stepsNeededList) / float(num_trials)
...
class RandomWalkRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        # The only difference from the Standard Robot
        self.setRobotDirection(random.randint(0, 360))
        newPosition = self.position.getNewPosition(self.direction, self.speed)
