...
#
    ...
    steps = 300
    trialResults = [[] for i in range(steps)]
    for i in range(numTrials):
        viruses = [SimpleVirus(maxBirthProb, clearProb) for j in range(numViruses)]
        patient = Patient(viruses, maxPop)
        for step in range(steps):
            trialResults[step].append(patient.update())
    results = [sum(t) / float(numTrials) for t in trialResults]
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        # TODO
        resistantToAllDrugs = all([self.isResistantTo(drug) for drug in activeDrugs])
        probReproduce = random.random() < self.maxBirthProb * (1 - popDensity)
        reproduce = resistantToAllDrugs and probReproduce
        if reproduce:
            self.resistances = {k:v if random.random() > self.mutProb else not v for k, v in self.resistances.items()}
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        # TODO
        return len([virus for virus in self.viruses if all(virus.isResistantTo(d) for d in drugResist)])
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    steps = 300
    treatOnStep = 150
    trialResultsTot = [[] for i in range(steps)]
    trialResultsRes = [[] for i in range(steps)]
    for i in range(numTrials):
        viruses = [ResistantVirus(maxBirthProb, clearProb, resistances.copy(), mutProb) for j in range(numViruses)]
